...
if (isset($_POST["sendotpbtn"])) {
    ...
    $stmt = $conn->prepare("SELECT * FROM admins WHERE email LIKE ?");
    ...
    if ($result->num_rows > 0) {
        ...
        try {
            ...
            $mail->Body = "<p>Dear Admin,</p><br><p>Your OTP Number Is <b>$otp</b></p><br><p>Thank You!</p>";

            $mail->send();
            $_SESSION["step"] = 2;
            echo '<script>alert("OTP Already Sent To Your Email...\\nPlease Check!"); location.href="admin_forgotpwd.php?step=2";</script>';
        } catch (Exception $e) {
            echo '<script>alert("Something went wrong...Please Try Later");</script>';
        }
    } else {
        echo '<script>alert("Your Email Has Not Found!"); location.href="admin_forgotpwd.php?step=1";</script>';
    }
    $stmt->close();
}
...
if (isset($_POST["verifyotpbtn"])) {
    ...
    if ($inputOtp == $_SESSION["otp"]) {
        $_SESSION["step"] = 3;
        echo '<script>location.href="admin_forgotpwd.php?step=3";</script>';
    } else {
        echo '<script>alert("Your OTP Is Not Correct!"); location.href="admin_forgotpwd.php?step=2";</script>';
    }
}
...
if (isset($_POST["resetbtn"])) {
    ...
    $stmt = $conn->prepare("UPDATE admins SET password = ? WHERE email LIKE ?");
    ...
    unset($_SESSION["step"]);
    echo '<script>alert("Your Password Has Been Reset...\\nPlease Login"); location.href="admin_login.php";</script>';
}
?>
...
<div class="container">
    ...
    <form method="post" action="admin_forgotpwd.php">
        <div class="mb-3 mt-3">
